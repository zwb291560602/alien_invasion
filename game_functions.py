...
def check_keydown_events(event,ai_settings,screen,ship,bullets):
	...
	if event.key == pygame.K_RIGHT:#右箭头
		ship.moving_right = True#当True时，alien_invasion.py调用ship.update向右移动
	elif event.key == pygame.K_LEFT:#左键头
...
bullets):
	"""响应按键和鼠标事件"""
	#监视键盘和鼠标事件，所有键盘和鼠标事件都将促使for 循环运行
...
def update_screen(ai_settings,screen,stats,sb,ship,aliens,bullets,play_button):
	...
	#绘制飞船
	ship.blitme()
	#绘制外星人,对编组调用draw() 时，Pygame自动绘制编组的每个元素
	aliens.draw(screen)
	#显示得分
	sb.show_score()
...
aliens,bullets):
	...
	if collisions:#记分
		for aliens in collisions.values():#可能一颗子弹消灭多个外星人
...
def create_fleet(ai_settings,screen,ship,aliens):
	"""创建外星人群"""
	#创建一个外星人，并计算一行可容乃多少个外星人,外星人间距为外星人宽度
...
def get_number_rows(ai_settings,ship_height,alien_height):
	"""计算屏幕可容纳多少行外星人"""
	#垂直空间：将屏幕高度减去第一行外星人的上边距（外星人高度）、
	#减去飞船的高度、减去最初外星人群与飞船的距离（外星人高度的两倍，留出射杀空白）
